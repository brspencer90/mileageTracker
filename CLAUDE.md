...
"Mileage tracker v2" — a self-hosted fuel/fill-up tracker (single user, multi-vehicle-ready). FastAPI + SQL Server (pyodbc) backend serving a Vite + React + TypeScript PWA, designed for sub-30-second fill-up logging from a phone. Target deployment is a Synology DS220+ via **Docker + CI/CD** (build-and-pull: GitHub Actions → GHCR → Watchtower auto-pulls on the NAS), reached from the phone via Tailscale. Local development runs everything on the dev machine.

Product backlog and decisions: `docs/PRODUCT_PLAN.md`. Build plan: `docs/IMPLEMENTATION_PLAN.md`. **NAS/Docker deployment: `docs/DEPLOY_SYNOLOGY.md`** (Dockerfile + docker-compose.yml at repo root; `.github/workflows/` ci.yml + deploy.yml). The database is **Microsoft SQL Server** at `192.168.0.20` (v2 tables `vehicles`/`fillups` in the `mileageTracker` DB, alongside — but not touching — the legacy `mileage` table). The container is stateless: it gets the `sqlss_conn_str` connection string at runtime from a git-ignored env file (the image bundles the ODBC Driver 18). Personal data files are kept out of the image via `.dockerignore`. (v2 ran on SQLite through 2026-07-24; migrated to SQL Server for single-database backup — the data-layer is now pyodbc/T-SQL.) `/api/version` and the UI's `ver.` footer report the running image's `GIT_SHA`. (The earlier "no Docker, SynoCommunity Python + Task Scheduler" plan was reversed in favor of this CI/CD pipeline.)
...
- `backend/app/` — FastAPI app. `db.py` opens per-request **pyodbc** connections to SQL Server and applies `app/schema.sql` (idempotent `IF OBJECT_ID IS NULL CREATE TABLE …`, incl. a **filtered unique index** `WHERE mileage IS NOT NULL` — SQL Server's plain UNIQUE would reject multiple NULL-mileage pending fills) at startup via `run_schema`. `queries.py` holds all T-SQL; MPG/MPF are **derived at read time in Python** (`_derive`, partial-fill-aware — a partial fill's fuel rolls into the next full fill) — never stored, so edits/deletes/imports can't corrupt neighboring rows. `config.py` reads the un-prefixed `sqlss_conn_str` from repo-root `.env`. (The old SQLite `migrations/*.sql` + `data/mileage.db` are dead; SQLite snapshots remain in `data/backups/` as the pre-SQL-Server source.) Routers: `vehicles`, `fillups` (CRUD + `/api/fillups/context`, the one-call endpoint powering the form's live MPG preview and station quick-picks), `stats` (chart series). `main.py` also serves `frontend/dist` with an SPA fallback when it exists.
...
- Dev and prod both use the SQL Server at `192.168.0.20` via `sqlss_conn_str` in repo-root `.env` (gitignored). **Tests run against an isolated `mileageTracker_test` database** on the same server — `conftest.py` derives its connection by swapping the DB name and asserts it's on `_test` before writing (the production `mileageTracker` DB is never touched by tests). `pytest` therefore needs the SQL Server reachable (CI spins up a `mssql` service container). One-time SQLite→SQL Server data migration: `scripts/migrate_sqlite_to_sqlserver.py`.
